...
import pack.jdbc.FileBean;

public class FUO {
	
	public boolean mtdUpload(HttpServletRequest request, FileBean fileBean) {

		boolean chk = false;
		String saveFolder = "D:\\Java_Web_AI\\silsp\\JSP\\p09_Quiz_Upload_JDBC_DTO\\src\\main\\webapp\\fileStorage";
		...
		try{
			...
			fileBean.setFsub(subject);
			fileBean.setfContent(content);
			fileBean.setfRealName(oriFileName);
			fileBean.setfUploadName(upLoadName);
			fileBean.setFilesize(fileSize);
			fileBean.setRemoteIP(remoteIP);
